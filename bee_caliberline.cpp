...
QRectF bee_caliberline::boundingRect() const
{
    ...
    else
    {
        ...
        return QRectF(re.topLeft().x()-5,re.topLeft().y()-5,re.width()+10,re.height()+10);
    }
}
...
void bee_caliberline::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    else
    {
        ...
        qreal arrow_length = getdistance_2(pt_begin,pt_end)/16;
        QPointF P1 = QPointF(end.x()+arrow_length*cos(angle-ExtRefArrowDegrees), end.y()+arrow_length*sin(angle-ExtRefArrowDegrees));
        QPointF P2 = QPointF(end.x()+arrow_length*cos(angle+ExtRefArrowDegrees), end.y()+arrow_length*sin(angle+ExtRefArrowDegrees));
        mPen.setWidth(0);
        painter->setPen(mPen);
