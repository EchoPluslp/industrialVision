...
QtDrawingPaperEditor::QtDrawingPaperEditor(QWidget* parent,  QImage imageData)
    : QWidget(parent)
{
...
void QtDrawingPaperEditor::DrawPaper(QPainter& painter) {
    ...
    //获取图片
    QRect target(paper_x_, paper_y_, draw_width, draw_height); //建立目标矩形，该区域是显示图像的目的地
    painter.drawImage(target, dataInfo);
   }
